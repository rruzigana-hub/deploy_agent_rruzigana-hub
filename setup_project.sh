...
# ─────────────────────────────────────────────
#  Project: Automated Project Bootstrapping
#  Script:  setup_project.sh
#  Author:  RUZIGANA
# ─────────────────────────────────────────────

# ── 1. Get user input for project name ────────
echo "=========================================="
...
cleanup() {
    ...
    if [ -d "$PROJECT_DIR" ]; then
        ...
        rm -rf "$PROJECT_DIR"
        echo "🗑️  Incomplete directory '${PROJECT_DIR}' deleted."
    fi
...
if [ "$update_choice" = "yes" ] || [ "$update_choice" = "y" ]; then
    ...
    if ! echo "$new_warning" | grep -qE '^[0-9]+$'; then
        echo "ERROR: Warning threshold must be a number. Using default 75."
        new_warning=75
    fi
    ...
    if ! echo "$new_failure" | grep -qE '^[0-9]+$'; then
        echo "ERROR: Failure threshold must be a number. Using default 50."
        new_failure=50
    fi
    ...
    echo "ℹ️  Keeping default thresholds (warning: 75%, failure: 50%)."
fi

# ── 9. Environment / Health Check ─────────────
echo ""
echo "=========================================="
...
if python3 --version > /dev/null 2>&1; then
    ...
    echo "⚠️  WARNING: python3 is NOT installed. The attendance checker will not run."
fi
